...
# - `.accepts` hash of `mediaType -> addOperation` that is used for Drag-and-Dropping onto
#    the content in the workspace list
#
...
define ['backbone'], (Backbone) ->
  ...
  return new class MediaTypes extends Backbone.Collection
    ...
    model: class MediaType extends Backbone.Model
      sync: -> throw new Error('This model cannot be syncd')
    ...
    sync: -> throw new Error('This collection cannot be syncd')
